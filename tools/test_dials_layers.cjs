...
(async () => {
  ...
  const cases = [['SKX 3.0 Case', 'skx'], ['Vintage Case', 'vtg'], ['Classic Retro Case', 'cl'], ['AP Case', 'ap']];
  for (const [cat, tag] of cases) {
    ...
    await (await p.$('#imgStack')).screenshot({ path: '/tmp/d_' + TAG + '_' + tag + '.png' });
  }
  console.log('40x: ' + (bad.length ? bad.slice(0, 4).join(' ') : 'none'));
